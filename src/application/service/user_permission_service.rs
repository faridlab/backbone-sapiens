//! Application service for UserPermission entities
//!
//! Generated by backbone-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `user_permission_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::presentation::dto::{CreateUserPermissionDto, UpdateUserPermissionDto};
use crate::domain::entity::UserPermission;
use crate::infrastructure::persistence::UserPermissionRepository;

/// Application service for UserPermission entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `user_permission_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct UserPermissionServiceCustom {
///     inner: Arc<UserPermissionService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type UserPermissionService = GenericCrudService<
    UserPermission,
    CreateUserPermissionDto,
    UpdateUserPermissionDto,
    UserPermissionRepository,
>;

// <<< CUSTOM
// END CUSTOM
