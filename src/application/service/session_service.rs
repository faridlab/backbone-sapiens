//! Application service for Session entities
//!
//! Generated by backbone-schema. Do not edit manually.
//!
//! Type alias over `GenericCrudService` — all CRUD logic lives in backbone-core.
//! For custom behaviour, create `session_service_custom.rs` (preserved on regen).

use backbone_core::GenericCrudService;

use crate::presentation::dto::{CreateSessionDto, UpdateSessionDto};
use crate::domain::entity::Session;
use crate::infrastructure::persistence::SessionRepository;

/// Application service for Session entities.
///
/// All 12 standard CRUD operations (list, create, get, update, patch,
/// soft_delete, restore, empty_trash, bulk_create, upsert, find_by_id,
/// list_deleted) are provided by `GenericCrudService`.
///
/// Custom extensions go in `session_service_custom.rs`:
///
/// ```rust,ignore
/// pub struct SessionServiceCustom {
///     inner: Arc<SessionService>,
///     // add domain-specific dependencies here
/// }
/// ```
pub type SessionService = GenericCrudService<
    Session,
    CreateSessionDto,
    UpdateSessionDto,
    SessionRepository,
>;

// <<< CUSTOM
// END CUSTOM
