//! Bulk Operations for SystemSettings aggregate
//!
...
/// Bulk operations handler for SystemSettings
pub struct SystemSettingsBulkOperations<R: SystemSettingsRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: SystemSettingsRepository> SystemSettingsBulkOperations<R> {
    ...
    /// Bulk create system_settings entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<SystemSettings>) -> Result<BulkOperationResult<SystemSettings>> {
    ...
    /// Bulk update system_settings entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<SystemSettings>) -> Result<BulkOperationResult<SystemSettings>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete system_settings entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete system_settings entities by IDs
    ...
    /// Bulk restore soft-deleted system_settings entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<SystemSettings>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
