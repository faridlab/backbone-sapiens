//! Bulk Operations for PasswordPolicy aggregate
//!
...
use crate::domain::repositories::PasswordPolicyRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for PasswordPolicy
pub struct PasswordPolicyBulkOperations<R: PasswordPolicyRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: PasswordPolicyRepository> PasswordPolicyBulkOperations<R> {
    ...
    /// Bulk create password_policy entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<PasswordPolicy>) -> Result<BulkOperationResult<PasswordPolicy>> {
    ...
    /// Bulk update password_policy entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<PasswordPolicy>) -> Result<BulkOperationResult<PasswordPolicy>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete password_policy entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete password_policy entities by IDs
    ...
    /// Bulk restore soft-deleted password_policy entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<PasswordPolicy>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
