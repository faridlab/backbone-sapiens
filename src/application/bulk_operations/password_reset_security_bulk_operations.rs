//! Bulk Operations for PasswordResetSecurity aggregate
//!
...
use crate::domain::repositories::PasswordResetSecurityRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for PasswordResetSecurity
pub struct PasswordResetSecurityBulkOperations<R: PasswordResetSecurityRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: PasswordResetSecurityRepository> PasswordResetSecurityBulkOperations<R> {
    ...
    /// Bulk create password_reset_security entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<PasswordResetSecurity>) -> Result<BulkOperationResult<PasswordResetSecurity>> {
    ...
    /// Bulk update password_reset_security entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<PasswordResetSecurity>) -> Result<BulkOperationResult<PasswordResetSecurity>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete password_reset_security entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete password_reset_security entities by IDs
    ...
    /// Bulk restore soft-deleted password_reset_security entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<PasswordResetSecurity>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
