//! Bulk Operations for SessionLimit aggregate
//!
...
use crate::domain::repositories::SessionLimitRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for SessionLimit
pub struct SessionLimitBulkOperations<R: SessionLimitRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: SessionLimitRepository> SessionLimitBulkOperations<R> {
    ...
    /// Bulk create session_limit entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<SessionLimit>) -> Result<BulkOperationResult<SessionLimit>> {
    ...
    /// Bulk update session_limit entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<SessionLimit>) -> Result<BulkOperationResult<SessionLimit>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete session_limit entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete session_limit entities by IDs
    ...
    /// Bulk restore soft-deleted session_limit entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<SessionLimit>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
