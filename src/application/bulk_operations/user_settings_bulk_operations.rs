//! Bulk Operations for UserSettings aggregate
//!
...
/// Bulk operations handler for UserSettings
pub struct UserSettingsBulkOperations<R: UserSettingsRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: UserSettingsRepository> UserSettingsBulkOperations<R> {
    ...
    /// Bulk create user_settings entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<UserSettings>) -> Result<BulkOperationResult<UserSettings>> {
    ...
    /// Bulk update user_settings entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<UserSettings>) -> Result<BulkOperationResult<UserSettings>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete user_settings entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete user_settings entities by IDs
    ...
    /// Bulk restore soft-deleted user_settings entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<UserSettings>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
