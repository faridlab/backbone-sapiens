//! Bulk Operations for EmailVerificationToken aggregate
//!
...
use crate::domain::repositories::EmailVerificationTokenRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for EmailVerificationToken
pub struct EmailVerificationTokenBulkOperations<R: EmailVerificationTokenRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: EmailVerificationTokenRepository> EmailVerificationTokenBulkOperations<R> {
    ...
    /// Bulk create email_verification_token entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<EmailVerificationToken>) -> Result<BulkOperationResult<EmailVerificationToken>> {
    ...
    /// Bulk update email_verification_token entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<EmailVerificationToken>) -> Result<BulkOperationResult<EmailVerificationToken>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete email_verification_token entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete email_verification_token entities by IDs
    ...
    /// Bulk restore soft-deleted email_verification_token entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<EmailVerificationToken>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
