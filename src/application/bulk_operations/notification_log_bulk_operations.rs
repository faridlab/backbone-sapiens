//! Bulk Operations for NotificationLog aggregate
//!
...
use crate::domain::repositories::NotificationLogRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for NotificationLog
pub struct NotificationLogBulkOperations<R: NotificationLogRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: NotificationLogRepository> NotificationLogBulkOperations<R> {
    ...
    /// Bulk create notification_log entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<NotificationLog>) -> Result<BulkOperationResult<NotificationLog>> {
    ...
    /// Bulk update notification_log entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<NotificationLog>) -> Result<BulkOperationResult<NotificationLog>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete notification_log entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete notification_log entities by IDs
    ...
    /// Bulk restore soft-deleted notification_log entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<NotificationLog>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
