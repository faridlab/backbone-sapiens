//! Bulk Operations for TemporaryPermission aggregate
//!
...
use crate::domain::repositories::TemporaryPermissionRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for TemporaryPermission
pub struct TemporaryPermissionBulkOperations<R: TemporaryPermissionRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: TemporaryPermissionRepository> TemporaryPermissionBulkOperations<R> {
    ...
    /// Bulk create temporary_permission entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<TemporaryPermission>) -> Result<BulkOperationResult<TemporaryPermission>> {
    ...
    /// Bulk update temporary_permission entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<TemporaryPermission>) -> Result<BulkOperationResult<TemporaryPermission>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete temporary_permission entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete temporary_permission entities by IDs
    ...
    /// Bulk restore soft-deleted temporary_permission entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<TemporaryPermission>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
