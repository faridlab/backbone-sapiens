//! Bulk Operations for WorkflowDefinition aggregate
//!
...
use crate::domain::repositories::WorkflowDefinitionRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for WorkflowDefinition
pub struct WorkflowDefinitionBulkOperations<R: WorkflowDefinitionRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: WorkflowDefinitionRepository> WorkflowDefinitionBulkOperations<R> {
    ...
    /// Bulk create workflow_definition entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<WorkflowDefinition>) -> Result<BulkOperationResult<WorkflowDefinition>> {
    ...
    /// Bulk update workflow_definition entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<WorkflowDefinition>) -> Result<BulkOperationResult<WorkflowDefinition>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete workflow_definition entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete workflow_definition entities by IDs
    ...
    /// Bulk restore soft-deleted workflow_definition entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<WorkflowDefinition>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
