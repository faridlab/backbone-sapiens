//! Bulk Operations for PasswordExpirationSettings aggregate
//!
...
use crate::domain::repositories::PasswordExpirationSettingsRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for PasswordExpirationSettings
pub struct PasswordExpirationSettingsBulkOperations<R: PasswordExpirationSettingsRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: PasswordExpirationSettingsRepository> PasswordExpirationSettingsBulkOperations<R> {
    ...
    /// Bulk create password_expiration_settings entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<PasswordExpirationSettings>) -> Result<BulkOperationResult<PasswordExpirationSettings>> {
    ...
    /// Bulk update password_expiration_settings entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<PasswordExpirationSettings>) -> Result<BulkOperationResult<PasswordExpirationSettings>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete password_expiration_settings entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete password_expiration_settings entities by IDs
    ...
    /// Bulk restore soft-deleted password_expiration_settings entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<PasswordExpirationSettings>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
