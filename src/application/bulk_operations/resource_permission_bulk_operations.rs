//! Bulk Operations for ResourcePermission aggregate
//!
...
use crate::domain::repositories::ResourcePermissionRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for ResourcePermission
pub struct ResourcePermissionBulkOperations<R: ResourcePermissionRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: ResourcePermissionRepository> ResourcePermissionBulkOperations<R> {
    ...
    /// Bulk create resource_permission entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<ResourcePermission>) -> Result<BulkOperationResult<ResourcePermission>> {
    ...
    /// Bulk update resource_permission entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<ResourcePermission>) -> Result<BulkOperationResult<ResourcePermission>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete resource_permission entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete resource_permission entities by IDs
    ...
    /// Bulk restore soft-deleted resource_permission entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<ResourcePermission>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
