//! Bulk Operations for Notification aggregate
//!
...
/// Bulk operations handler for Notification
pub struct NotificationBulkOperations<R: NotificationRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: NotificationRepository> NotificationBulkOperations<R> {
    ...
    /// Bulk create notification entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<Notification>) -> Result<BulkOperationResult<Notification>> {
    ...
    /// Bulk update notification entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<Notification>) -> Result<BulkOperationResult<Notification>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete notification entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete notification entities by IDs
    ...
    /// Bulk restore soft-deleted notification entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<Notification>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
