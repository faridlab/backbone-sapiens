//! Bulk Operations for SecurityEvent aggregate
//!
...
use crate::domain::repositories::SecurityEventRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for SecurityEvent
pub struct SecurityEventBulkOperations<R: SecurityEventRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: SecurityEventRepository> SecurityEventBulkOperations<R> {
    ...
    /// Bulk create security_event entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<SecurityEvent>) -> Result<BulkOperationResult<SecurityEvent>> {
    ...
    /// Bulk update security_event entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<SecurityEvent>) -> Result<BulkOperationResult<SecurityEvent>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete security_event entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete security_event entities by IDs
    ...
    /// Bulk restore soft-deleted security_event entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<SecurityEvent>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
