//! Bulk Operations for WorkflowStep aggregate
//!
...
use crate::domain::repositories::WorkflowStepRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for WorkflowStep
pub struct WorkflowStepBulkOperations<R: WorkflowStepRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: WorkflowStepRepository> WorkflowStepBulkOperations<R> {
    ...
    /// Bulk create workflow_step entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<WorkflowStep>) -> Result<BulkOperationResult<WorkflowStep>> {
    ...
    /// Bulk update workflow_step entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<WorkflowStep>) -> Result<BulkOperationResult<WorkflowStep>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete workflow_step entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete workflow_step entities by IDs
    ...
    /// Bulk restore soft-deleted workflow_step entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<WorkflowStep>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
