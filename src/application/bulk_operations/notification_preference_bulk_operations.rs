//! Bulk Operations for NotificationPreference aggregate
//!
...
use crate::domain::repositories::NotificationPreferenceRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for NotificationPreference
pub struct NotificationPreferenceBulkOperations<R: NotificationPreferenceRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: NotificationPreferenceRepository> NotificationPreferenceBulkOperations<R> {
    ...
    /// Bulk create notification_preference entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<NotificationPreference>) -> Result<BulkOperationResult<NotificationPreference>> {
    ...
    /// Bulk update notification_preference entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<NotificationPreference>) -> Result<BulkOperationResult<NotificationPreference>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete notification_preference entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete notification_preference entities by IDs
    ...
    /// Bulk restore soft-deleted notification_preference entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<NotificationPreference>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
