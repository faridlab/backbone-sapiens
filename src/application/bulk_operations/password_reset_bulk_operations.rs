//! Bulk Operations for PasswordReset aggregate
//!
...
use crate::domain::repositories::PasswordResetRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for PasswordReset
pub struct PasswordResetBulkOperations<R: PasswordResetRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: PasswordResetRepository> PasswordResetBulkOperations<R> {
    ...
    /// Bulk create password_reset entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<PasswordReset>) -> Result<BulkOperationResult<PasswordReset>> {
    ...
    /// Bulk update password_reset entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<PasswordReset>) -> Result<BulkOperationResult<PasswordReset>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete password_reset entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete password_reset entities by IDs
    ...
    /// Bulk restore soft-deleted password_reset entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<PasswordReset>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
