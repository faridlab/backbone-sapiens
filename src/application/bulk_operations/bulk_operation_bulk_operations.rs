//! Bulk Operations for BulkOperation aggregate
//!
...
use crate::domain::repositories::BulkOperationRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for BulkOperation
pub struct BulkOperationBulkOperations<R: BulkOperationRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: BulkOperationRepository> BulkOperationBulkOperations<R> {
    ...
    /// Bulk create bulk_operation entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<BulkOperation>) -> Result<BulkOperationResult<BulkOperation>> {
    ...
    /// Bulk update bulk_operation entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<BulkOperation>) -> Result<BulkOperationResult<BulkOperation>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete bulk_operation entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete bulk_operation entities by IDs
    ...
    /// Bulk restore soft-deleted bulk_operation entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<BulkOperation>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
