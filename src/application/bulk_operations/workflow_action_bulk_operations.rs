//! Bulk Operations for WorkflowAction aggregate
//!
...
use crate::domain::repositories::WorkflowActionRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for WorkflowAction
pub struct WorkflowActionBulkOperations<R: WorkflowActionRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: WorkflowActionRepository> WorkflowActionBulkOperations<R> {
    ...
    /// Bulk create workflow_action entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<WorkflowAction>) -> Result<BulkOperationResult<WorkflowAction>> {
    ...
    /// Bulk update workflow_action entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<WorkflowAction>) -> Result<BulkOperationResult<WorkflowAction>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete workflow_action entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete workflow_action entities by IDs
    ...
    /// Bulk restore soft-deleted workflow_action entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<WorkflowAction>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
