//! Bulk Operations for Profile aggregate
//!
...
/// Bulk operations handler for Profile
pub struct ProfileBulkOperations<R: ProfileRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: ProfileRepository> ProfileBulkOperations<R> {
    ...
    /// Bulk create profile entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<Profile>) -> Result<BulkOperationResult<Profile>> {
    ...
    /// Bulk update profile entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<Profile>) -> Result<BulkOperationResult<Profile>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete profile entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete profile entities by IDs
    ...
    /// Bulk restore soft-deleted profile entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<Profile>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
