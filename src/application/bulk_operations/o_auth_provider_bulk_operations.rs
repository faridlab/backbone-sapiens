//! Bulk Operations for OAuthProvider aggregate
//!
...
use crate::domain::repositories::OAuthProviderRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for OAuthProvider
pub struct OAuthProviderBulkOperations<R: OAuthProviderRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: OAuthProviderRepository> OAuthProviderBulkOperations<R> {
    ...
    /// Bulk create o_auth_provider entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<OAuthProvider>) -> Result<BulkOperationResult<OAuthProvider>> {
    ...
    /// Bulk update o_auth_provider entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<OAuthProvider>) -> Result<BulkOperationResult<OAuthProvider>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete o_auth_provider entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete o_auth_provider entities by IDs
    ...
    /// Bulk restore soft-deleted o_auth_provider entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<OAuthProvider>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
