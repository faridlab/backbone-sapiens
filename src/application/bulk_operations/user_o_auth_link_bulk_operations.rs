//! Bulk Operations for UserOAuthLink aggregate
//!
...
use crate::domain::repositories::UserOAuthLinkRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for UserOAuthLink
pub struct UserOAuthLinkBulkOperations<R: UserOAuthLinkRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: UserOAuthLinkRepository> UserOAuthLinkBulkOperations<R> {
    ...
    /// Bulk create user_o_auth_link entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<UserOAuthLink>) -> Result<BulkOperationResult<UserOAuthLink>> {
    ...
    /// Bulk update user_o_auth_link entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<UserOAuthLink>) -> Result<BulkOperationResult<UserOAuthLink>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete user_o_auth_link entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete user_o_auth_link entities by IDs
    ...
    /// Bulk restore soft-deleted user_o_auth_link entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<UserOAuthLink>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
