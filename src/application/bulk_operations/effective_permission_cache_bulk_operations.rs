//! Bulk Operations for EffectivePermissionCache aggregate
//!
...
use crate::domain::repositories::EffectivePermissionCacheRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for EffectivePermissionCache
pub struct EffectivePermissionCacheBulkOperations<R: EffectivePermissionCacheRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: EffectivePermissionCacheRepository> EffectivePermissionCacheBulkOperations<R> {
    ...
    /// Bulk create effective_permission_cache entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<EffectivePermissionCache>) -> Result<BulkOperationResult<EffectivePermissionCache>> {
    ...
    /// Bulk update effective_permission_cache entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<EffectivePermissionCache>) -> Result<BulkOperationResult<EffectivePermissionCache>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete effective_permission_cache entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete effective_permission_cache entities by IDs
    ...
    /// Bulk restore soft-deleted effective_permission_cache entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<EffectivePermissionCache>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
