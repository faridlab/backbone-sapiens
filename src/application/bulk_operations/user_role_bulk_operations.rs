//! Bulk Operations for UserRole aggregate
//!
...
use crate::domain::repositories::UserRoleRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for UserRole
pub struct UserRoleBulkOperations<R: UserRoleRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: UserRoleRepository> UserRoleBulkOperations<R> {
    ...
    /// Bulk create user_role entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<UserRole>) -> Result<BulkOperationResult<UserRole>> {
    ...
    /// Bulk update user_role entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<UserRole>) -> Result<BulkOperationResult<UserRole>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete user_role entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete user_role entities by IDs
    ...
    /// Bulk restore soft-deleted user_role entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<UserRole>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
