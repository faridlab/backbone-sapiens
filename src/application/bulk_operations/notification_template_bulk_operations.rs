//! Bulk Operations for NotificationTemplate aggregate
//!
...
use crate::domain::repositories::NotificationTemplateRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for NotificationTemplate
pub struct NotificationTemplateBulkOperations<R: NotificationTemplateRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: NotificationTemplateRepository> NotificationTemplateBulkOperations<R> {
    ...
    /// Bulk create notification_template entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<NotificationTemplate>) -> Result<BulkOperationResult<NotificationTemplate>> {
    ...
    /// Bulk update notification_template entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<NotificationTemplate>) -> Result<BulkOperationResult<NotificationTemplate>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete notification_template entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete notification_template entities by IDs
    ...
    /// Bulk restore soft-deleted notification_template entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<NotificationTemplate>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
