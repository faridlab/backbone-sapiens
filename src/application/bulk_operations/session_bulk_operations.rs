//! Bulk Operations for Session aggregate
//!
...
/// Bulk operations handler for Session
pub struct SessionBulkOperations<R: SessionRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: SessionRepository> SessionBulkOperations<R> {
    ...
    /// Bulk create session entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<Session>) -> Result<BulkOperationResult<Session>> {
    ...
    /// Bulk update session entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<Session>) -> Result<BulkOperationResult<Session>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete session entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete session entities by IDs
    ...
    /// Bulk restore soft-deleted session entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<Session>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
