//! Bulk Operations for BackupCode aggregate
//!
...
use crate::domain::repositories::BackupCodeRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for BackupCode
pub struct BackupCodeBulkOperations<R: BackupCodeRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: BackupCodeRepository> BackupCodeBulkOperations<R> {
    ...
    /// Bulk create backup_code entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<BackupCode>) -> Result<BulkOperationResult<BackupCode>> {
    ...
    /// Bulk update backup_code entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<BackupCode>) -> Result<BulkOperationResult<BackupCode>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete backup_code entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete backup_code entities by IDs
    ...
    /// Bulk restore soft-deleted backup_code entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<BackupCode>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
