//! Bulk Operations for PasswordHistory aggregate
//!
...
/// Bulk operations handler for PasswordHistory
pub struct PasswordHistoryBulkOperations<R: PasswordHistoryRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: PasswordHistoryRepository> PasswordHistoryBulkOperations<R> {
    ...
    /// Bulk create password_history entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<PasswordHistory>) -> Result<BulkOperationResult<PasswordHistory>> {
    ...
    /// Bulk update password_history entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<PasswordHistory>) -> Result<BulkOperationResult<PasswordHistory>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete password_history entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete password_history entities by IDs
    ...
    /// Bulk restore soft-deleted password_history entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<PasswordHistory>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
