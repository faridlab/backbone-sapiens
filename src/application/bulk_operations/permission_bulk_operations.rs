//! Bulk Operations for Permission aggregate
//!
...
/// Bulk operations handler for Permission
pub struct PermissionBulkOperations<R: PermissionRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: PermissionRepository> PermissionBulkOperations<R> {
    ...
    /// Bulk create permission entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<Permission>) -> Result<BulkOperationResult<Permission>> {
    ...
    /// Bulk update permission entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<Permission>) -> Result<BulkOperationResult<Permission>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete permission entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete permission entities by IDs
    ...
    /// Bulk restore soft-deleted permission entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<Permission>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
