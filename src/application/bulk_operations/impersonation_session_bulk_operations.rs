//! Bulk Operations for ImpersonationSession aggregate
//!
...
use crate::domain::repositories::ImpersonationSessionRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for ImpersonationSession
pub struct ImpersonationSessionBulkOperations<R: ImpersonationSessionRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: ImpersonationSessionRepository> ImpersonationSessionBulkOperations<R> {
    ...
    /// Bulk create impersonation_session entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<ImpersonationSession>) -> Result<BulkOperationResult<ImpersonationSession>> {
    ...
    /// Bulk update impersonation_session entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<ImpersonationSession>) -> Result<BulkOperationResult<ImpersonationSession>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete impersonation_session entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete impersonation_session entities by IDs
    ...
    /// Bulk restore soft-deleted impersonation_session entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<ImpersonationSession>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
