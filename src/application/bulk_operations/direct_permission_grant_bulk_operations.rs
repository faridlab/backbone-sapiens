//! Bulk Operations for DirectPermissionGrant aggregate
//!
...
use crate::domain::repositories::DirectPermissionGrantRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for DirectPermissionGrant
pub struct DirectPermissionGrantBulkOperations<R: DirectPermissionGrantRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: DirectPermissionGrantRepository> DirectPermissionGrantBulkOperations<R> {
    ...
    /// Bulk create direct_permission_grant entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<DirectPermissionGrant>) -> Result<BulkOperationResult<DirectPermissionGrant>> {
    ...
    /// Bulk update direct_permission_grant entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<DirectPermissionGrant>) -> Result<BulkOperationResult<DirectPermissionGrant>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete direct_permission_grant entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete direct_permission_grant entities by IDs
    ...
    /// Bulk restore soft-deleted direct_permission_grant entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<DirectPermissionGrant>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
