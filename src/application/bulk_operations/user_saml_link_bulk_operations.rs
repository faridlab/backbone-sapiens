//! Bulk Operations for UserSAMLLink aggregate
//!
...
use crate::domain::repositories::UserSAMLLinkRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for UserSAMLLink
pub struct UserSAMLLinkBulkOperations<R: UserSAMLLinkRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: UserSAMLLinkRepository> UserSAMLLinkBulkOperations<R> {
    ...
    /// Bulk create user_saml_link entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<UserSAMLLink>) -> Result<BulkOperationResult<UserSAMLLink>> {
    ...
    /// Bulk update user_saml_link entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<UserSAMLLink>) -> Result<BulkOperationResult<UserSAMLLink>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete user_saml_link entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete user_saml_link entities by IDs
    ...
    /// Bulk restore soft-deleted user_saml_link entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<UserSAMLLink>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
