//! Bulk Operations for PasswordResetToken aggregate
//!
...
/// Bulk operations handler for PasswordResetToken
pub struct PasswordResetTokenBulkOperations<R: PasswordResetTokenRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: PasswordResetTokenRepository> PasswordResetTokenBulkOperations<R> {
    ...
    /// Bulk create password_reset_token entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<PasswordResetToken>) -> Result<BulkOperationResult<PasswordResetToken>> {
    ...
    /// Bulk update password_reset_token entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<PasswordResetToken>) -> Result<BulkOperationResult<PasswordResetToken>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete password_reset_token entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete password_reset_token entities by IDs
    ...
    /// Bulk restore soft-deleted password_reset_token entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<PasswordResetToken>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
