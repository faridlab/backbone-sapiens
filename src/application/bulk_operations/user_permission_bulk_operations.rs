//! Bulk Operations for UserPermission aggregate
//!
...
/// Bulk operations handler for UserPermission
pub struct UserPermissionBulkOperations<R: UserPermissionRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: UserPermissionRepository> UserPermissionBulkOperations<R> {
    ...
    /// Bulk create user_permission entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<UserPermission>) -> Result<BulkOperationResult<UserPermission>> {
    ...
    /// Bulk update user_permission entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<UserPermission>) -> Result<BulkOperationResult<UserPermission>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete user_permission entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete user_permission entities by IDs
    ...
    /// Bulk restore soft-deleted user_permission entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<UserPermission>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
