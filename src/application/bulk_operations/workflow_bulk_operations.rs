//! Bulk Operations for Workflow aggregate
//!
...
/// Bulk operations handler for Workflow
pub struct WorkflowBulkOperations<R: WorkflowRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: WorkflowRepository> WorkflowBulkOperations<R> {
    ...
    /// Bulk create workflow entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<Workflow>) -> Result<BulkOperationResult<Workflow>> {
    ...
    /// Bulk update workflow entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<Workflow>) -> Result<BulkOperationResult<Workflow>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete workflow entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete workflow entities by IDs
    ...
    /// Bulk restore soft-deleted workflow entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<Workflow>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
