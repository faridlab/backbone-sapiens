//! Bulk Operations for PermissionConflict aggregate
//!
...
use crate::domain::repositories::PermissionConflictRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for PermissionConflict
pub struct PermissionConflictBulkOperations<R: PermissionConflictRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: PermissionConflictRepository> PermissionConflictBulkOperations<R> {
    ...
    /// Bulk create permission_conflict entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<PermissionConflict>) -> Result<BulkOperationResult<PermissionConflict>> {
    ...
    /// Bulk update permission_conflict entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<PermissionConflict>) -> Result<BulkOperationResult<PermissionConflict>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete permission_conflict entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete permission_conflict entities by IDs
    ...
    /// Bulk restore soft-deleted permission_conflict entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<PermissionConflict>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
