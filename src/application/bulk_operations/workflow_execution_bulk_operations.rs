//! Bulk Operations for WorkflowExecution aggregate
//!
...
use crate::domain::repositories::WorkflowExecutionRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for WorkflowExecution
pub struct WorkflowExecutionBulkOperations<R: WorkflowExecutionRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: WorkflowExecutionRepository> WorkflowExecutionBulkOperations<R> {
    ...
    /// Bulk create workflow_execution entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<WorkflowExecution>) -> Result<BulkOperationResult<WorkflowExecution>> {
    ...
    /// Bulk update workflow_execution entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<WorkflowExecution>) -> Result<BulkOperationResult<WorkflowExecution>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete workflow_execution entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete workflow_execution entities by IDs
    ...
    /// Bulk restore soft-deleted workflow_execution entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<WorkflowExecution>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
