//! Bulk Operations for MFABackupCode aggregate
//!
...
use crate::domain::repositories::MFABackupCodeRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for MFABackupCode
pub struct MFABackupCodeBulkOperations<R: MFABackupCodeRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: MFABackupCodeRepository> MFABackupCodeBulkOperations<R> {
    ...
    /// Bulk create mfa_backup_code entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<MFABackupCode>) -> Result<BulkOperationResult<MFABackupCode>> {
    ...
    /// Bulk update mfa_backup_code entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<MFABackupCode>) -> Result<BulkOperationResult<MFABackupCode>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete mfa_backup_code entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete mfa_backup_code entities by IDs
    ...
    /// Bulk restore soft-deleted mfa_backup_code entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<MFABackupCode>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
