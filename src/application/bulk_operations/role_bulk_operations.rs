//! Bulk Operations for Role aggregate
//!
...
/// Bulk operations handler for Role
pub struct RoleBulkOperations<R: RoleRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: RoleRepository> RoleBulkOperations<R> {
    ...
    /// Bulk create role entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<Role>) -> Result<BulkOperationResult<Role>> {
    ...
    /// Bulk update role entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<Role>) -> Result<BulkOperationResult<Role>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete role entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete role entities by IDs
    ...
    /// Bulk restore soft-deleted role entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<Role>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
