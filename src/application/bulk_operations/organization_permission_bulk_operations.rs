//! Bulk Operations for OrganizationPermission aggregate
//!
...
use crate::domain::repositories::OrganizationPermissionRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for OrganizationPermission
pub struct OrganizationPermissionBulkOperations<R: OrganizationPermissionRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: OrganizationPermissionRepository> OrganizationPermissionBulkOperations<R> {
    ...
    /// Bulk create organization_permission entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<OrganizationPermission>) -> Result<BulkOperationResult<OrganizationPermission>> {
    ...
    /// Bulk update organization_permission entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<OrganizationPermission>) -> Result<BulkOperationResult<OrganizationPermission>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete organization_permission entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete organization_permission entities by IDs
    ...
    /// Bulk restore soft-deleted organization_permission entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<OrganizationPermission>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
