//! Bulk Operations for DeviceTrust aggregate
//!
...
use crate::domain::repositories::DeviceTrustRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for DeviceTrust
pub struct DeviceTrustBulkOperations<R: DeviceTrustRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: DeviceTrustRepository> DeviceTrustBulkOperations<R> {
    ...
    /// Bulk create device_trust entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<DeviceTrust>) -> Result<BulkOperationResult<DeviceTrust>> {
    ...
    /// Bulk update device_trust entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<DeviceTrust>) -> Result<BulkOperationResult<DeviceTrust>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete device_trust entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete device_trust entities by IDs
    ...
    /// Bulk restore soft-deleted device_trust entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<DeviceTrust>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
