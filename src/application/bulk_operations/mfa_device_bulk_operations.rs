//! Bulk Operations for MFADevice aggregate
//!
...
use crate::domain::repositories::MFADeviceRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for MFADevice
pub struct MFADeviceBulkOperations<R: MFADeviceRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: MFADeviceRepository> MFADeviceBulkOperations<R> {
    ...
    /// Bulk create mfa_device entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<MFADevice>) -> Result<BulkOperationResult<MFADevice>> {
    ...
    /// Bulk update mfa_device entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<MFADevice>) -> Result<BulkOperationResult<MFADevice>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete mfa_device entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete mfa_device entities by IDs
    ...
    /// Bulk restore soft-deleted mfa_device entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<MFADevice>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
