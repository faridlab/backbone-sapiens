//! Bulk Operations for RolePermission aggregate
//!
...
use crate::domain::repositories::RolePermissionRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for RolePermission
pub struct RolePermissionBulkOperations<R: RolePermissionRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: RolePermissionRepository> RolePermissionBulkOperations<R> {
    ...
    /// Bulk create role_permission entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<RolePermission>) -> Result<BulkOperationResult<RolePermission>> {
    ...
    /// Bulk update role_permission entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<RolePermission>) -> Result<BulkOperationResult<RolePermission>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete role_permission entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete role_permission entities by IDs
    ...
    /// Bulk restore soft-deleted role_permission entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<RolePermission>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
