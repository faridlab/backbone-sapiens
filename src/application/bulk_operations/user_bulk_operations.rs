//! Bulk Operations for User aggregate
//!
...
/// Bulk operations handler for User
pub struct UserBulkOperations<R: UserRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: UserRepository> UserBulkOperations<R> {
    ...
    /// Bulk create user entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<User>) -> Result<BulkOperationResult<User>> {
    ...
    /// Bulk update user entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<User>) -> Result<BulkOperationResult<User>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete user entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete user entities by IDs
    ...
    /// Bulk restore soft-deleted user entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<User>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
