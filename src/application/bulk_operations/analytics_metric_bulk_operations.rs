//! Bulk Operations for AnalyticsMetric aggregate
//!
...
use crate::domain::repositories::AnalyticsMetricRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for AnalyticsMetric
pub struct AnalyticsMetricBulkOperations<R: AnalyticsMetricRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: AnalyticsMetricRepository> AnalyticsMetricBulkOperations<R> {
    ...
    /// Bulk create analytics_metric entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<AnalyticsMetric>) -> Result<BulkOperationResult<AnalyticsMetric>> {
    ...
    /// Bulk update analytics_metric entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<AnalyticsMetric>) -> Result<BulkOperationResult<AnalyticsMetric>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete analytics_metric entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete analytics_metric entities by IDs
    ...
    /// Bulk restore soft-deleted analytics_metric entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<AnalyticsMetric>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
