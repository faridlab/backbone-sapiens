//! Bulk Operations for DataExport aggregate
//!
...
use crate::domain::repositories::DataExportRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for DataExport
pub struct DataExportBulkOperations<R: DataExportRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: DataExportRepository> DataExportBulkOperations<R> {
    ...
    /// Bulk create data_export entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<DataExport>) -> Result<BulkOperationResult<DataExport>> {
    ...
    /// Bulk update data_export entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<DataExport>) -> Result<BulkOperationResult<DataExport>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete data_export entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete data_export entities by IDs
    ...
    /// Bulk restore soft-deleted data_export entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<DataExport>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
