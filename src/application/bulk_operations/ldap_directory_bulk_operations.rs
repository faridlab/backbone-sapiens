//! Bulk Operations for LDAPDirectory aggregate
//!
...
use crate::domain::repositories::LDAPDirectoryRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for LDAPDirectory
pub struct LDAPDirectoryBulkOperations<R: LDAPDirectoryRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: LDAPDirectoryRepository> LDAPDirectoryBulkOperations<R> {
    ...
    /// Bulk create ldap_directory entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<LDAPDirectory>) -> Result<BulkOperationResult<LDAPDirectory>> {
    ...
    /// Bulk update ldap_directory entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<LDAPDirectory>) -> Result<BulkOperationResult<LDAPDirectory>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete ldap_directory entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete ldap_directory entities by IDs
    ...
    /// Bulk restore soft-deleted ldap_directory entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<LDAPDirectory>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
