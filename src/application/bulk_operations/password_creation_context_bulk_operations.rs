//! Bulk Operations for PasswordCreationContext aggregate
//!
...
use crate::domain::repositories::PasswordCreationContextRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for PasswordCreationContext
pub struct PasswordCreationContextBulkOperations<R: PasswordCreationContextRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: PasswordCreationContextRepository> PasswordCreationContextBulkOperations<R> {
    ...
    /// Bulk create password_creation_context entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<PasswordCreationContext>) -> Result<BulkOperationResult<PasswordCreationContext>> {
    ...
    /// Bulk update password_creation_context entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<PasswordCreationContext>) -> Result<BulkOperationResult<PasswordCreationContext>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete password_creation_context entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete password_creation_context entities by IDs
    ...
    /// Bulk restore soft-deleted password_creation_context entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<PasswordCreationContext>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
