//! Bulk Operations for RoleAssignment aggregate
//!
...
use crate::domain::repositories::RoleAssignmentRepository;
use super::{BulkOperationConfig, BulkOperationProgress, BulkOperationResult, BulkOperationError};

/// Bulk operations handler for RoleAssignment
pub struct RoleAssignmentBulkOperations<R: RoleAssignmentRepository> {
    repository: Arc<R>,
    config: BulkOperationConfig,
}

impl<R: RoleAssignmentRepository> RoleAssignmentBulkOperations<R> {
    ...
    /// Bulk create role_assignment entities with batching and error recovery
    pub async fn bulk_create(&self, entities: Vec<RoleAssignment>) -> Result<BulkOperationResult<RoleAssignment>> {
    ...
    /// Bulk update role_assignment entities with batching and error recovery
    pub async fn bulk_update(&self, entities: Vec<RoleAssignment>) -> Result<BulkOperationResult<RoleAssignment>> {
        let total = entities.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
    ...
    /// Bulk delete role_assignment entities by IDs with batching and error recovery
    ...
    /// Bulk soft delete role_assignment entities by IDs
    ...
    /// Bulk restore soft-deleted role_assignment entities by IDs
    pub async fn bulk_restore(&self, ids: Vec<String>) -> Result<BulkOperationResult<RoleAssignment>> {
        let total = ids.len();
        let mut progress = BulkOperationProgress::new(total, self.config.batch_size);
        let mut result = BulkOperationResult::new(progress.clone());
