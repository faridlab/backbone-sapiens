# Database Migrations for sapiens

This directory contains SQLx database migrations for the sapiens module. These migrations create a comprehensive database schema with support for:
...
Creates the main `sapiens` table with:
...
### 002_create_sapiens_events_table.sql
Creates the event store table `sapiens_events` with:
...
### 003_create_sapiens_audit_table.sql
Creates the audit trail table `sapiens_audit` with:
...
### 004_create_sapiens_indexes_and_triggers.sql
...
### 005_create_sapiens_views.sql
Creates optimized database views:
- `sapiens_active` - Only active entities
- `sapiens_stats` - Materialized statistics view
- `sapiens_with_activity` - Entities with recent activity
- `sapiens_audit_summary` - Audit summary per entity
- `sapiens_event_timeline` - Chronological event view
- `sapiens_audit_timeline` - Chronological audit view
- `sapiens_by_metadata` - Entities with extracted metadata
- `sapiens_performance_metrics` - Daily performance metrics
...
sapiens
├── Main Table: sapiens
├── Event Store: sapiens_events
├── Audit Trail: sapiens_audit
├── Views (Optimized):
│   ├── sapiens_active
│   ├── sapiens_stats (materialized)
│   ├── sapiens_with_activity
│   ├── sapiens_audit_summary
│   ├── sapiens_event_timeline
│   ├── sapiens_audit_timeline
│   ├── sapiens_by_metadata
│   └── sapiens_performance_metrics (materialized)
└── Functions:
    ├── search_sapiens()
    ├── get_sapiens_statistics()
    ├── refresh_sapiens_materialized_views()
    └── schedule_sapiens_refresh()
```
...
CREATE POLICY sapiens_read_policy ON sapiens
    FOR SELECT USING (deleted_at IS NULL);

ALTER TABLE sapiens ENABLE ROW LEVEL SECURITY;
```
...
FROM pg_matviews
WHERE matviewname LIKE 'sapiens_%';

-- Check index usage
SELECT schemaname, tablename, indexname, idx_scan, idx_tup_read
FROM pg_stat_user_indexes
WHERE tablename LIKE 'sapiens_%';
```
