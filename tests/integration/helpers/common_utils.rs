...
impl CommonUtils {
    ...
    /// Generate a valid test password hash that meets 60+ character requirement
    pub fn generate_password_hash(&self) -> String {
        // Generate a mock Argon2id hash that meets the 60+ character validation
        format!("$argon2id$v=19$m=19456,t=2,p=1${}${}{}",
                self.generate_id("salt"),
                self.generate_id("hash"),
                "x".repeat(20)) // Add padding to ensure 60+ chars
    }
