...
pub struct TestResources {
    ...
    /// Role IDs created during tests
    pub role_ids: Vec<String>,

    /// Permission IDs created during tests
    pub permission_ids: Vec<String>,

    /// Cache keys to delete
    pub cache_keys: Vec<String>,

    /// Session IDs created during tests
...
impl TestResources {
    ...
    pub fn track_user(&mut self, id: impl Into<String>) {
        self.user_ids.push(id.into());
    }

    /// Track a role ID for cleanup
    pub fn track_role(&mut self, id: impl Into<String>) {
        self.role_ids.push(id.into());
    }

    /// Track a permission ID for cleanup
    pub fn track_permission(&mut self, id: impl Into<String>) {
        self.permission_ids.push(id.into());
    }
    ...
    pub fn track_session(&mut self, id: impl Into<String>) {
        self.session_ids.push(id.into());
    }
...
impl TestSetupManager {
    ...
    pub async fn teardown(&mut self) -> Result<(), TestError> {
        if self.config.verbose {
            println!(
                "[Teardown] Cleaning up: {} users, {} roles, {} permissions",
                self.resources.user_ids.len(),
                self.resources.role_ids.len(),
                self.resources.permission_ids.len()
            );
        }
    ...
    pub fn generate_test_username(&self, prefix: &str) -> String {
...
mod tests {
    ...
    fn test_resource_tracking() {
        ...
        resources.track_user("user-1");
        resources.track_role("role-1");
        resources.track_permission("perm-1");

        assert_eq!(resources.user_ids.len(), 1);
        assert_eq!(resources.role_ids.len(), 1);
        assert_eq!(resources.permission_ids.len(), 1);

        resources.clear();
        assert!(resources.user_ids.is_empty());
    }
}
