...
///
/// Provides common HTTP request methods and response handling.
///
/// # Example
///
/// ```ignore
/// let api_test = ApiTest::new("user_api_test", "http://localhost:3000");
/// let response = api_test.get("/api/v1/users", None).await?;
...
/// Builder for complex API requests
...
impl<'a> RequestBuilder<'a> {
    ...
    pub fn new(api_test: &'a ApiTest, method: Method, endpoint: impl Into<String>) -> Self {
        Self {
            api_test,
            method,
    ...
    /// Execute the request
    pub async fn send(self) -> Result<ApiResponse, TestError> {
        self.api_test
            .request(self.method, &self.endpoint, Some(self.headers), self.body)
            .await
    }
}
