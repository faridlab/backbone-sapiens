...
///
/// Captures all relevant information about a single test execution
/// for debugging and reporting purposes.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TestResult {
    /// Name of the test (e.g., "Get User - Success")
...
///
/// Provides computed properties for test statistics.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct TestSuiteResult {
    ...
    /// When the suite ended (None if still running)
...
impl TestSuiteResult {
    ...
    pub fn save_individual_results(&self, dir: &std::path::Path) -> Result<(), std::io::Error> {
        std::fs::create_dir_all(dir)?;

        for (i, result) in self.results.iter().enumerate() {
            let filename = format!("test_{:03}_{}.json", i, sanitize_filename(&result.test_name));
...
///
/// Defines the contract for test lifecycle: setup → run_tests → teardown
///
/// # Example
///
/// ```ignore
...
///     test_data_ids: Vec<String>,
/// }
///
/// #[async_trait]
/// impl Test for MyApiTest {
///     fn name(&self) -> &str { "my_api_test" }
///
...
pub trait Test: Send + Sync {
    ...
    /// Initialize resources (DB connections, test data)
    async fn setup(&mut self) -> Result<(), TestError>;

    /// Cleanup resources (delete test data, close connections)
    async fn teardown(&mut self) -> Result<(), TestError>;
    ...
    ///
    /// This is the entry point for running a test suite.
    async fn execute(&mut self) -> TestSuiteResult {
        ...
        // Teardown phase (always run, even if tests failed)
...
/// Helper struct for common test assertions
...
impl Assertions {
    ...
    pub fn assert_true(value: bool, message: &str) -> Result<(), String> {
        if !value {
            return Err(format!("{}: expected true, got false", message));
        }
        Ok(())
    }

    /// Assert that an Option is Some
    pub fn assert_some<T>(value: &Option<T>, message: &str) -> Result<(), String> {
        if value.is_none() {
            return Err(format!("{}: expected Some, got None", message));
        }
        Ok(())
    }

    /// Assert that a Result is Ok
    pub fn assert_ok<T, E: fmt::Debug>(value: &Result<T, E>, message: &str) -> Result<(), String> {
        if let Err(e) = value {
            return Err(format!("{}: expected Ok, got Err({:?})", message, e));
        }
        Ok(())
    }
}
