...
impl<G: TestDataGenerator> GenericCrudTest<G> {
    ...
    pub async fn test_create(&mut self) -> TestResult {
        ...
        let payload = self.generator.generate_create_payload(&self.utils);
    ...
    pub async fn test_update(&self, id: &str) -> TestResult {
        ...
        let payload = self.generator.generate_update_payload(id, &self.utils);
    ...
    pub async fn run_all(&mut self) -> Vec<TestResult> {
        ...
        // Batch operations (exercised on the created entities;
        // soft-delete then restore so they remain for cleanup below)
